...
#TA
#think of other groups than just being positive and negative, group words that have the same meaning fabulous = delicious
c(sort(names(traindata[,c(-1 : -8)])))
#look at different cities use of words
#include city in the model

#Professor
#when you get predictions, plot the histogram
#transform predictions that are above and below 5 and 1, set them to those
#potentially round to 1, 2, 3, 4, 5; consider giving half star ratings

#lm(y ~ city+ words)

#calibrate the effectiveness of the word by dividing by total number of words

#grouping: be careful of simply adding word counts together, instead
#chunk them together into if words exist, then pow_word = 1 or something else depending on strength required

#there are a lot of neutral words that might not mean anything alone, but combined with other words, they become positives

#we can manually categorize words

#think about how you would write a review, when you would write the review, why you would write a review
#what kind of restaurant it is, who would go there
#write a small for loop to pick out certain features

#ensemble model, average of predictions
#you know the true value star rating of out data, so run a regression between stars and all of the predictions

#superlearner package



#right now we have the average star of each word
